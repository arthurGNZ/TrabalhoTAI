...
<body>
  ...
  <div class="page">
    <h1> Listagem de contatos<h1>
        <form action='contato.php' class="row gx-3 gy-2 align-items-center" method="POST">
          <div class="col-sm-3">
            <label class="visually-hidden" for="specificSizeInputName">Name</label>
            <input type="text" class="form-control" placeholder="Pesquisar" name="nome" id="nome">
          </div>
          <div class="col-sm-3">
            <label class="visually-hidden" for="specificSizeSelect">Preference</label>
            <select class="form-select" id="opcoes" name="opcoes">
              <option selected value="nome">Tipo</option>
        ...
        <?php
        if (!empty($_POST['nome'])) {
          $result = $objBD->search($_POST['nome'], $_POST['opcoes']);
        } else {
          $result = $objBD->select();
        }
